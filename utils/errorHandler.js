...
const errorHandling = async (e) => {
  ...
  logEvent(LOG_LEVELS.info, RESPONSE_CODES.LOG_MESSAGE_ONLY, `Handling error`)
  ...
  if (stringError.includes("Rate limit exceeded")) {
    const errorDescription = await stringError.match(/Rate limit exceeded, retry in \d/)
    const timeout = errorDescription[0].match(/\d+/)
